...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('SQLALCHEMY_DATABASE_URI', 'sqlite:///fan_profiles.db')
...
TESSERACT_PATH = r'C:\Program Files\Tesseract-OCR\tesseract.exe' # <<< AJUSTE SEU CAMINHO AQUI!!!
try:
    ...
    logger.error(f"Tesseract OCR NÃO encontrado. Configure o caminho manualmente se o OCR falhar. Erro: {e}")
...
async def validate_esports_link(url: str, keywords: List[str]) -> str:
    ...
    try:
        async with httpx.AsyncClient(follow_redirects=True, timeout=10.0) as client:
            ...
            found_keyword = any(keyword in page_text for keyword in keywords if keyword) # Verifica se keyword não é vazia
...
async def edit_profile(): # Rota continua async por causa da validação de links
    ...
    if form.validate_on_submit():
        ...
        logger.info(f"Objeto FanProfile {'novo' if was_new else 'existente'} populado com dados do form.")
        ...
        if file:
             try:
                 ...
                 if pytesseract.pytesseract.tesseract_cmd or pytesseract.get_tesseract_version(): # Verifica se Tesseract está ok
                     try:
                         ...
                         profile.doc_ocr_result = ocr_text.strip() if ocr_text else "OCR não encontrou texto."
                         logger.info(f"OCR concluído. Texto (início): {profile.doc_ocr_result[:100]}")
                     except pytesseract.TesseractNotFoundError:
                          logger.error("ERRO OCR: Tesseract não encontrado durante execução.")
                     ...
                     logger.warning("OCR não executado pois Tesseract não foi encontrado/configurado.")
        ...
        if raw_links_text:
            keywords_base = []
            if profile.favorite_teams: keywords_base.extend([t.strip().lower() for t in profile.favorite_teams.split(',') if t.strip()])
            if profile.favorite_games: keywords_base.extend([g.strip().lower() for g in profile.favorite_games.split(',') if g.strip()])
            ...
            if urls_to_check:
                ...
                validation_summary = [f"• {url}: {status}" for url, status in zip(urls_to_check, results)]
